...
function findAll({ paciente_id, status, page = 1, limit = 20 } = {}) {
  let sql = `
    SELECT r.*, p.nome_completo as paciente_nome, b.nome as beneficio_nome, b.categoria as beneficio_categoria
    FROM requisicoes r
...
function findById(id) {
  return getDb().prepare(`
    SELECT r.*, p.nome_completo as paciente_nome, b.nome as beneficio_nome, b.categoria
    FROM requisicoes r
    JOIN pacientes p ON p.id = r.paciente_id
    JOIN beneficios b ON b.id = r.beneficio_id
    WHERE r.id = ?
...
function create(data) {
  const info = getDb().prepare(
    'INSERT INTO requisicoes (paciente_id, beneficio_id, quantidade, observacoes, status, solicitado_por) VALUES (?, ?, ?, ?, ?, ?)'
  ).run(data.paciente_id, data.beneficio_id, data.quantidade, data.observacoes, 'solicitada', data.solicitado_por);
...
function findAprovadasByPacienteId(pacienteId) {
  return getDb().prepare(`
    ...
    FROM requisicoes r JOIN beneficios b ON b.id = r.beneficio_id
    WHERE r.paciente_id = ? AND r.status IN ('aprovada','entregue')
...
function countByBeneficio(inicio, fim) {
  return getDb().prepare(`
    SELECT b.nome, b.categoria, COUNT(r.id) as total
    FROM beneficios b
    LEFT JOIN requisicoes r ON r.beneficio_id = b.id
      AND r.status IN ('aprovada','entregue')
      AND r.data_solicitacao >= ? AND r.data_solicitacao <= ?
    WHERE b.ativo = 1
