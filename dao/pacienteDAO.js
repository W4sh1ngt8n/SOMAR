...
function findById(id) {
  return getDb().prepare('SELECT * FROM pacientes WHERE id = ?').get(id);
}

function findByCpf(cpf) {
  return getDb().prepare('SELECT * FROM pacientes WHERE cpf = ?').get(cpf);
}
...
function updateQRCode(id, qrPath) {
  getDb().prepare('UPDATE pacientes SET qr_code_path = ? WHERE id = ?').run(qrPath, id);
}

function countByStatus() {
  return getDb().prepare(`
    SELECT status_cadastro, COUNT(*) as total FROM pacientes GROUP BY status_cadastro
  `).all();
}

function findRecentes(limit = 5) {
  return getDb().prepare('SELECT id, nome_completo, status_cadastro, criado_em FROM pacientes ORDER BY criado_em DESC LIMIT ?').all(limit);
}
