...
function findById(id) {
  return getDb().prepare('SELECT * FROM pre_cadastros WHERE id = ?').get(id);
}

function create(data) {
  const info = getDb().prepare(
    'INSERT INTO pre_cadastros (nome_completo, telefone, whatsapp, motivo_busca, documentos_solicitados, responsavel_recepcao) VALUES (?, ?, ?, ?, ?, ?)'
  ).run(data.nome_completo, data.telefone, data.whatsapp, data.motivo_busca, data.documentos_solicitados, data.responsavel_recepcao);
...
function markValidated(id) {
  return update(id, { status: 'validado', documentos_recebidos: 1 });
}
...
function countPendentes() {
  return getDb().prepare("SELECT COUNT(*) as total FROM pre_cadastros WHERE status = 'aguardando'").get().total;
}
