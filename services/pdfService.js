...
function footer(doc) {
  doc.fontSize(8).fillColor('#95a5a6')
    .text(`Documento gerado automaticamente em ${new Date().toLocaleString('pt-BR')} - Sistema Fundação SOMAR`, 50, 800, { align: 'center', width: 495 });
}
...
function generateFichaPaciente(paciente, acompanhamentos, beneficios, patologias, medicamentos, qrCodePath) {
  ...
  acompanhamentos.forEach(a => doc.text(`[${a.data}] ${a.tipo.toUpperCase()} - ${a.descricao}${a.profissional ? ' (Prof: ' + a.profissional + ')' : ''}${a.proximo_retorno ? ' | Próx. retorno: ' + a.proximo_retorno : ''}`));
  doc.moveDown();
