...
function findAll() { return getDb().prepare('SELECT * FROM medicamentos ORDER BY nome').all(); }
function findById(id) { return getDb().prepare('SELECT * FROM medicamentos WHERE id = ?').get(id); }
...
function update(id, data) {
  ...
  ['nome','principio_ativo','apresentacao','dosagem_padrao','ativo'].forEach(f => { if (data[f] !== undefined) { fields.push(`${f} = ?`); values.push(data[f]); } });
  if (fields.length === 0) return findById(id);
  values.push(id);
  getDb().prepare(`UPDATE medicamentos SET ${fields.join(', ')} WHERE id = ?`).run(...values);
  return findById(id);
}
function remove(id) { getDb().prepare('DELETE FROM medicamentos WHERE id = ?').run(id); }
function findByPacienteId(pacienteId) {
  return getDb().prepare(`SELECT pm.*, m.nome, m.principio_ativo, m.apresentacao FROM paciente_medicamentos pm JOIN medicamentos m ON m.id = pm.medicamento_id WHERE pm.paciente_id = ?`).all(pacienteId);
}
...
function desvincularPaciente(id) { getDb().prepare('UPDATE paciente_medicamentos SET ativo = 0 WHERE id = ?').run(id); }
