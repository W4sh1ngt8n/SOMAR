...
function findAll() { return getDb().prepare('SELECT * FROM beneficios ORDER BY nome').all(); }
function findById(id) { return getDb().prepare('SELECT * FROM beneficios WHERE id = ?').get(id); }
...
function remove(id) { getDb().prepare('DELETE FROM beneficios WHERE id = ?').run(id); }
