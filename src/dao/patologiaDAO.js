...
function findAll() { return getDb().prepare('SELECT * FROM patologias ORDER BY nome').all(); }
function findById(id) { return getDb().prepare('SELECT * FROM patologias WHERE id = ?').get(id); }
...
function remove(id) { getDb().prepare('DELETE FROM patologias WHERE id = ?').run(id); }
function findByPacienteId(pacienteId) {
  return getDb().prepare(`SELECT pp.*, p.nome, p.gravidade FROM paciente_patologias pp JOIN patologias p ON p.id = pp.patologia_id WHERE pp.paciente_id = ?`).all(pacienteId);
}
...
function desvincularPaciente(pacienteId, patologiaId) { getDb().prepare('DELETE FROM paciente_patologias WHERE paciente_id = ? AND patologia_id = ?').run(pacienteId, patologiaId); }
