    ...
    document.getElementById('dashContent').innerHTML = `
      ...
      <div class="card" style="margin-top:24px">
        ...
        <table class="data-table">
          <thead><tr><th>Nome</th><th>Status</th><th>Cadastrado em</th></tr></thead>
          <tbody>${recentesHtml}</tbody>
        </table>
      </div>`;
  } catch (err) {
    document.getElementById('dashContent').innerHTML = `<p>Erro: ${err.message}</p>`;
  }
}
...
async function loadPacientes() {
  ...
  try {
    ...
    const headers = [
      ...
      { key: 'cpf', label: 'CPF' },
      { key: 'status_cadastro', label: 'Status', badge: statusBadge },
      { key: 'status_tratamento', label: 'Tratamento' }
    ];
    const actions = (r) => [
      { label: 'Ver', class: 'btn-primary', onclick: () => `window.location.hash='#/paciente/${r.id}'` }
    ];
  ...
  try {
    ...
    document.getElementById('pacienteDetail').innerHTML = `
      ...
      <div class="card">
        ...
        <table class="data-table">
          <thead><tr><th>Data</th><th>Tipo</th><th>Descrição</th><th>Profissional</th><th>Próx. Retorno</th></tr></thead>
          <tbody>${acsHtml || '<tr><td colspan="5">Nenhum registro</td></tr>'}</tbody>
        </table>
      </div>
      <div class="card">
        <div class="card-title">Benefícios Concedidos</div>
        <table class="data-table">
          <thead><tr><th>Data</th><th>Benefício</th><th>Quantidade</th><th>Status</th></tr></thead>
          <tbody>${reqsHtml || '<tr><td colspan="4">Nenhum registro</td></tr>'}</tbody>
        </table>
      </div>`;
  } catch (err) {
    document.getElementById('pacienteDetail').innerHTML = `<p>Erro: ${err.message}</p>`;
  }
}
...
async function loadPreCadastros() {
  try {
    ...
    const actions = (r) => {
      ...
      if (r.status === 'validado') btns.push({ label: 'Converter', class: 'btn-primary', onclick: () => `converterPre(${r.id})` });
      return btns;
    };
...
async function validarPre(id) {
  try {
    await api.patch(`/pre-cadastros/${id}/validar`);
    showToast('Validado!', 'success');
    loadPreCadastros();
  } catch (e) {
...
async function converterPre(id) {
  ...
  try {
    await api.post(`/pre-cadastros/${id}/converter`, {
      nome_completo: nome,
      cpf: cpf || null,
      status_cadastro: 'triagem'
    });
        ...
        <div class="form-row">
          ...
          <div class="form-group">
            ...
            <select id="ac_tipo" required>
              ...
              <option value="escuta_inicial">Escuta Inicial</option>
            </select>
          </div>
        </div>
        <div class="form-row">
...
async function renderRequisicoes() {
  buildLayout('Requisições de Benefícios', `
    ...
    <div class="card">
      ...
      <div id="reqList"><p>Carregando...</p></div>
    </div>
  `);

  try {
    const [pacientes, beneficios] = await Promise.all([
      api.get('/pacientes?status_cadastro=assistido&limit=9999'),
      api.get('/beneficios')
    ]);
...
async function loadRequisicoes() {
  try {
    ...
    const headers = [
      ...
      { key: 'beneficio_nome', label: 'Benefício' },
...
async function aprovarReq(id) {
  try { await api.patch(`/requisicoes/${id}/aprovar`); showToast('Aprovado!', 'success'); loadRequisicoes(); }
  catch (e) { showToast(e.message, 'error'); }
}

async function negarReq(id) {
  try { await api.patch(`/requisicoes/${id}/negar`); showToast('Negado!', 'success'); loadRequisicoes(); }
  catch (e) { showToast(e.message, 'error'); }
}

async function entregarReq(id) {
  try { await api.patch(`/requisicoes/${id}/entregar`); showToast('Entregue!', 'success'); loadRequisicoes(); }
  catch (e) { showToast(e.message, 'error'); }
}

async function downloadComprovante(id) {
  try {
    const b = await api.download(`/requisicoes/${id}/comprovante-pdf`);
...
async function gerarRelAnual() {
  ...
  try {
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `relatorio_anual_${ano}.pdf`;
    a.click();
    URL.revokeObjectURL(url);
    showToast('Relatório gerado!', 'success');
  } catch (err) {
...
async function renderDocumentos() {
  buildLayout('Documentos', `
    ...
    <div class="card">
      ...
      <div id="docList"><p>Selecione um paciente para ver os documentos.</p></div>
    </div>
  `);
...
async function renderMedicamentos() {
  ...
  try {
    ...
    const headers = [
      ...
      { key: 'dosagem_padrao', label: 'Dosagem' }
    ];
...
async function renderUsuarios() {
  buildLayout('Usuários', `
    <div class="card">
      ...
      <form id="userForm">
        ...
        <div class="form-row">
          ...
          <div class="form-group">
            <label>Perfil *</label>
            <select id="user_perfil" required>
    ...
    <div class="card">
      ...
      <div id="userList"><p>Carregando...</p></div>
    </div>
  `);
