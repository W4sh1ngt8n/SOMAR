...
function listar(req, res) {
  ...
  if (req.user.perfil === 'visitante') {
    result.data = result.data.map(p => ({
      ...p, cpf: maskCPF(p.cpf, 'visitante'), telefone: maskPhone(p.telefone, 'visitante'),
      endereco: '*******', email: null,
    }));
  }
  res.json(result);
...
function criar(req, res) {
  ...
  if (data.cpf) {
    const existente = pacienteDAO.findByCpf(data.cpf);
    if (existente) return res.status(409).json({ erro: 'Já existe paciente com este CPF' });
  }
...
function atualizar(req, res) {
  const p = pacienteDAO.update(req.params.id, req.body);
...
function historico(req, res) {
  try {
    const historico = pacienteService.getHistoricoCompleto(req.params.id, req.user.perfil);
    res.json(historico);
