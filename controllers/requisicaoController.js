...
function buscar(req, res) {
  const r = requisicaoDAO.findById(req.params.id);
...
function criar(req, res) {
  ...
  const data = { ...req.body, solicitado_por: req.user.id };
  const r = requisicaoDAO.create(data);
  res.status(201).json(r);
}

function aprovar(req, res) {
  const r = requisicaoDAO.updateStatus(req.params.id, 'aprovada', req.user.id);
  if (!r) return res.status(404).json({ erro: 'Requisição não encontrada' });
  res.json(r);
}

function negar(req, res) {
  const r = requisicaoDAO.updateStatus(req.params.id, 'negada', req.user.id);
  if (!r) return res.status(404).json({ erro: 'Requisição não encontrada' });
  res.json(r);
}

function entregar(req, res) {
  const r = requisicaoDAO.updateStatus(req.params.id, 'entregue', req.user.id);
  if (!r) return res.status(404).json({ erro: 'Requisição não encontrada' });
  res.json(r);
}
...
async function comprovantePdf(req, res) {
  ...
  if (qrPath && fs.existsSync(qrPath)) {
    doc.text('QR Code de Validação:', 380, doc.y);
    doc.image(qrPath, 400, doc.y + 5, { width: 80 });
  }
