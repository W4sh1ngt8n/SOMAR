...
function buildLayout(pageTitle, contentHtml) {
  ...
  nav.innerHTML = navItems.map(i => `<a href="${i.href}" class="${window.location.hash === i.href ? 'active' : ''}"><span class="nav-icon">${i.icon}</span> ${i.label}</a>`).join('');
}
...
function renderTable(headers, rows, actions = null) {
  ...
  const trs = rows.map(row => {
    const tds = headers.map(h => {
      let val = row[h.key];
      if (h.badge && val) return `<td><span class="badge ${h.badge(val)}">${h.format ? h.format(val) : val}</span></td>`;
      return `<td>${h.format ? h.format(val) : (val ?? '-')}</td>`;
    }).join('');
