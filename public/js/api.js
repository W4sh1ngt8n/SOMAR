...
async function request(endpoint, options = {}) {
  ...
  if (response.status === 401) { sessionStorage.removeItem('somar_token'); sessionStorage.removeItem('somar_user'); window.location.hash = '#/login'; throw new Error('Sessão expirada'); }
...
async function uploadFile(endpoint, file, extraFields) {
  ...
  const r = await fetch(`${API_BASE}${endpoint}`, { method: 'POST', headers: { Authorization: `Bearer ${token}` }, body: formData });
